...
def format_sample_janus(piece, vl_chat_processor):
    sample = {
        ...
        'better_image': load_image(piece['better_image']),
        'worse_image': load_image(piece['worse_image']),
    }
    return sample
...
def tokenize_sample(vl_chat_processor, vl_gpt, vl_image_processor, formatted_sample):
    ...
    better_pixel_values = (
        vl_image_processor([formatted_sample['better_image']], return_tensors='pt')['pixel_values']
        .to(vl_gpt.device)
        .to(torch.bfloat16)
    )
    (
        better_quant,
        (better_vq_loss, better_commit_loss, better_entropy_loss),
    ...
    (
        worse_quant,
        (worse_vq_loss, worse_commit_loss, worse_entropy_loss),
    ...
    return {
        ...
        'worse_input_ids': worse_full_input_ids.to('cpu'),
        'task': 'generation',
    }
